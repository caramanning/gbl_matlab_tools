...
% including all samples from this month
asA = 1:length(LISA.Lat);
...
stns_to_exclude = ["CLIS-cast01"
    ...
    asA = [];
    for i = 1:length(stns_to_exclude)
        A = find(LISM.Station==stns_to_exclude(i));
        asA = [asA; A];
    end;
    W = 1:length(LISM.Station);
...
subplot(2,2,1)
hold on;
iA = find(LISA.Depth>0);
iO = find(LISO.Depth>0);
iM = find(LISM.Depth>0);
