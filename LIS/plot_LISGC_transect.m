...
%%
% replace the data very close to the bottom to better match the overall
% trend
DCH4_nmolkgiA(176,3) = DCH4_nmolkgiA(174,3);
DCH4_nmolkgiA(177:end,3) = DCH4_nmolkgiA(177,2);


%%DCH4_nmolkgiA(174:end,3) = DCH4_nmolkgiA(173,3);
...
clevelCH4 = [caCH4(1):2:100 110:10:caCH4(2)]; %CH4 colorbar levels

caN2O = [0 7]; % N2O axis limits
clevelN2O = [caN2O(1):0.15:caN2O(2)]; % N2O colorbar levels

caO2 = [-200 70];
clevelO2 = [caO2(1):5:caO2(2)];

caPDen = [16 18.8]; % PDen axis limits
clevelPDen = [caPDen(1):0.05:caPDen(2)]; % PDen colorbar levels
...
% cmap2 = interp1(x, cmapi, 1:512);
 
cmap2 = 'nebula';

fig=figure(24);
clf;
sp=tight_subplot(nr,nc,[.025 .02],[.08 .04],[.06 .05]);
%sp=tight_subplot(nr,nc,[.025 .025],[.08 .04],[.08 .04]);
set(gcf, 'PaperUnits', 'inches');
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperPosition', [0 0 12 8]);
...
cbar = colorbar;
cbar.Ticks = [25 50 100 200 400];
colormap(cmap2);
colorbar('off');


   % caxis(caCH4,[0 1])
   % colormap(caCH4,cmap2)
    % colormap(cmap2)
