...
foldername = "H:\Shared drives\Gas Biogeochemistry Lab\projects\CIRCA 2022 seed grant\deployment Aug 12";

fname_O2 =  'PME_O2_cc.mat';
filepath_O2 = fullfile(foldername,fname_O2);
load(filepath_O2);

fname_CTD =  'S12071_Aug12_Aug19.mat';
...
xti = datetime(2022,8,12,12,30,0):dt:datetime(2022,8,19,11,0,0);
...
N2_i(xti<datetime(2022,8,12,16,30,0))  = NaN;
DN2_i(xti<datetime(2022,8,12,16,30,0))  = NaN;
dat.n2sat(xti<datetime(2022,8,12,16,30,0))  = NaN;
...
print -dpng -r300 O2_N2_sat_Aug12_Aug19.png;
...
%plot(xti,T_i_PO);
%plot(xti,T_i_SO,'k');
%legend('PME', 'PO', 'SO', 'location','south');
ylabel('temp [^oC]');
axis tight;
ylim([21 29])
%xlim([t_start t_end]);
...
print -dpng -r300 O2eq_N2eq_Aug12_Aug19.png;

%%
xl = [datenum(2022,8,7) datenum(2022,8,11)];
figure(4)
clf; 
subplot(3,1,1)
hold on; box on;
...
print -dpng -r300 O2eq_N2eq_Aug12_Aug19_subset.png;


save O2_N2_Aug12_Aug19.mat xti S_i T_i O2_i N2_i TDGP_i

%%

ts = datetime(2022,8,13,6,0,0);
te = datetime(2022,8,18,6,0,0);
figure(4)
clf; 
x0=10;
y0=10;
...
title('eelgrass meadow')
plot(xti,T_i,'linewidth',1);
%plot(xti,T_i_PO);
%plot(xti,T_i_SO,'k');
%legend('PME', 'PO', 'SO', 'location','south');
...
hold on; box on;
set(gca,'fontname','arial')
%rectangle([datetime(2022,8,7,6,0,0) 360 datetime(2022,8,7,12,0,0) 100]);
%rectangle([1 1 100 100]);
plot(xti,N2_eq,'k')
plot(xti,N2_i,'color',[0 0.4470 0.7410],'linewidth',1);
%legend('TDGP', 'location','eastoutside');
ylabel('N_2 [\mumol kg^{-1}]');
axis tight;
ylim([360 460])
...
set(gca,'fontname','arial')
plot(xti,O2_eq,'k')
plot(xti,O2_i,'color',[0 0.4470 0.7410],'linewidth',1);
%legend('O_2', 'location','eastoutside');
ylabel('O_2 [\mumol kg^{-1}]');
axis tight;
ylim([100 350])
...
print -dpng -r300 O2eq_N2eq_Aug12_Aug19_subset.png;
print -dpdf -r300 O2eq_N2eq_Aug12_Aug19_subset.pdf;
print -depsc -r300 O2eq_N2eq_Aug12_Aug19_subset.eps;
